/*
  # Fix daily schedules migration

  1. Changes
    - Properly handle the transition to daily_schedules
...
-- First ensure the daily_schedules column exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 
    ...
    AND column_name = 'daily_schedules'
  ) THEN
    ALTER TABLE user_habits ADD COLUMN daily_schedules jsonb DEFAULT '[]'::jsonb;
  END IF;
END $$;
...
BEGIN
  ...
  IF schedules IS NULL THEN
    RETURN true; -- Allow null during migration
  END IF;
...
-- Drop the constraint if it exists to allow updates
ALTER TABLE user_habits DROP CONSTRAINT IF EXISTS valid_daily_schedules;

-- Initialize daily_schedules with default values for any null or empty values
UPDATE user_habits
SET daily_schedules = (
  SELECT jsonb_agg(
    jsonb_build_object(
