...
-- Create improved function with more flexible time handling
...
BEGIN
  ...
  IF NOT p_event_time::text ~ '^([0-1][0-9]|2[0-3]):[0-5][0-9](:[0-5][0-9](\.[0-9]+)?)?$' THEN
    RAISE EXCEPTION 'Invalid event time format. Expected HH:MM or HH:MM:SS';
  END IF;

  IF NOT p_reminder_time::text ~ '^([0-1][0-9]|2[0-3]):[0-5][0-9](:[0-5][0-9](\.[0-9]+)?)?$' THEN
    RAISE EXCEPTION 'Invalid reminder time format. Expected HH:MM or HH:MM:SS';
  END IF;
...
-- Add helpful comment
COMMENT ON FUNCTION schedule_habit_reminders IS 'Schedule a reminder for a habit with flexible time format validation (HH:MM or HH:MM:SS)';