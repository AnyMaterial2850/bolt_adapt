...
-- Create improved function with better time handling
...
BEGIN
  ...
  IF NOT p_event_time::text ~ '^([0-1][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9](\.[0-9]+)?$' THEN
    RAISE EXCEPTION 'Invalid event time format. Expected HH:MM:SS';
  END IF;

  IF NOT p_reminder_time::text ~ '^([0-1][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9](\.[0-9]+)?$' THEN
    RAISE EXCEPTION 'Invalid reminder time format. Expected HH:MM:SS';
  END IF;
...
-- Add helpful comment
COMMENT ON FUNCTION schedule_habit_reminders IS 'Schedule a reminder for a habit with strict time format validation';