...
import { ChevronDown, ChevronUp, XCircle, AlertCircle, CheckCircle, Info, Bug, Activity } from 'lucide-react';
...
export function DebugPanel() {
  ...
  const isHealthCheckVisible = useDebugStore((state) => state.isHealthCheckVisible);
  const setHealthCheckVisible = useDebugStore((state) => state.setHealthCheckVisible);
  const [isExpanded, setIsExpanded] = useState(true);
  const [filter, setFilter] = useState<string>('all');
  
  // Allow in production for health checks
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between p-2 border-b border-gray-700">
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className="text-xs bg-blue-700 px-2 py-1 rounded hover:bg-blue-600 flex items-center"
      ...
      {/* Health Check Panel */}
      <HealthCheckPanel 
        isVisible={isHealthCheckVisible} 
